...
| | |
| --- | --- |
| ![Taskbar widget with three pulsing dots and a badge reading 2/3: two of three open sessions working](docs/screenshots/taskbar-widget-working.png) | **Session badge and working dots** *(Windows widget)* — a box under the mark reads *working / open*: how many Claude Code sessions have a turn in progress, out of how many are open. It is there whenever the watch is on, `0/0` included. While a turn is in progress the mark gives way to three pulsing dots — Claude's own thinking sign — and the box breathes; the mark is back the moment nothing is working. |
| <img src="docs/screenshots/macos-menu-bar-working.png" alt="The macOS menu bar item: three pulsing dots, a box reading 2/3, then 5h (2:36) 29% · 7d 58%" width="420"> | **Session badge and working dots** *(macOS menu bar)* — the same box, drawn beside the mark rather than written into the row, and painted in the menu bar's own ink so it reads in dark and light alike. While a turn is in progress the mark gives way to three pulsing dots, as above. |
